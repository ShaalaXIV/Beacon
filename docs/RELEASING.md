# Releasing Compass safely

Compass has two independently versioned deliverables: the plugin ZIP and the server container. A
...
1. Bump the four-part plugin version in `Compass.Plugin.csproj`. Bump `ApiRoutes.ProtocolVersion`
...
3. Back up `/opt/portainer/compass/data` and verify the backup checksum and SQLite integrity.
4. Commit, tag `vX.Y.Z`, and publish `publish/Compass-X.Y.Z.zip` as the GitHub release asset.
5. Build immutable `compass-server:X.Y.Z` and `compass-backup:X.Y.Z` images from that exact tag.
