...
public sealed class ChronicleWindow : Window
{
    ...
    public ChronicleWindow(
        ...
        : base("The Chronicle###CompassChronicle")
    {
        this.config = config;
    ...
    private void DrawGalleryPopup()
    {
        ...
        ImGui.OpenPopup("Gallery###CompassGallery");

        var open = true;
        ImGui.SetNextWindowSizeConstraints(new Vector2(420, 300), new Vector2(1400, 1000));

        if (!ImGui.BeginPopupModal("Gallery###CompassGallery", ref open, ImGuiWindowFlags.None))
        {
            viewingGallery = null;
            return;
        }
