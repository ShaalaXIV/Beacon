...
/// <summary>
/// Talks to the Compass server. Every method returns rather than throws, because every one of these
/// calls happens behind a button that needs to show a reason when it does not work.
/// </summary>
public sealed class CompassApi(Configuration config) : IDisposable
{
        ...
        if (health.Protocol != ApiRoutes.ProtocolVersion)
        {
            return ApiResult<ServerHealth>.Failure(
                health.Protocol > ApiRoutes.ProtocolVersion
                    ? "This server is newer than your plugin. Update Compass to connect."
        ...
        CancellationToken ct)
    {
        if (authenticated && !config.HasAccount)
            return ApiResult<T>.Failure("No Compass account yet. Open the settings to create one.", HttpStatusCode.Unauthorized);
        ...
        try
        {
            ...
            if (body is not null)
            {
                request.Content = new StringContent(
                    JsonSerializer.Serialize(body, body.GetType(), CompassJson.Options),
                    Encoding.UTF8,
                    "application/json");
            }
    ...
    private async Task<ApiResult<T>> ReadAsync<T>(HttpResponseMessage response, CancellationToken ct)
    {
        ...
        try
        {
            var value = JsonSerializer.Deserialize<T>(payload, CompassJson.Options);
            return value is null
                ? ApiResult<T>.Failure("The server sent something Compass could not read.", response.StatusCode)
                : ApiResult<T>.Success(value);
        }
        catch (JsonException ex)
        {
            Svc.Log.Warning(ex, "Could not deserialise a {Type} response.", typeof(T).Name);
            return ApiResult<T>.Failure("The server sent something Compass could not read.", response.StatusCode);
        }
    }
    ...
    private static string ExtractError(string payload, HttpStatusCode status)
    {
        ...
        {
            HttpStatusCode.Unauthorized => "Your Compass key was not accepted.",
            ...
            HttpStatusCode.UpgradeRequired => "The Compass server requires a newer plugin. Update Compass and try again.",
    ...
    {
        Reachable = false;
        Svc.Log.Debug(ex, "Compass server unreachable.");
        return ApiResult<T>.Failure("Could not reach the Compass server.", HttpStatusCode.ServiceUnavailable);
    }
