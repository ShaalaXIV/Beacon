...
public sealed class ScreenshotService : IDisposable
{
    ...
    public async Task<byte[]?> CaptureAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            using var texture = await Svc.Textures.CreateFromImGuiViewportAsync(args, "Compass capture", ct);
        ...
        {
            Svc.Log.Warning(ex, "Compass: viewport capture failed.");
    ...
    public void PickFile(Action<byte[], string> onPicked)
    {
        ...
        dialogs.OpenFileDialog(
            ...
            (confirmed, paths) =>
            {
                ...
                {
                    Svc.Log.Warning(ex, "Compass: could not read the chosen screenshot.");
    ...
    private static string? DefaultScreenshotFolder()
    {
        ...
        {
            Svc.Log.Debug(ex, "Compass: could not resolve the screenshot folder.");
            return null;
        }
    }
    ...
    private static Guid? FindPngEncoder()
    {
        ...
        {
            Svc.Log.Warning(ex, "Compass: could not enumerate image encoders.");
            return null;
        }
    }
