...
public sealed class BeaconHubClient(Configuration config) : IDisposable
{
    ...
    public void Stop()
    {
        ...
        {
            Svc.Log.Debug(ex, "Compass: stopping the hub client threw.");
        }
    ...
    private async Task RunAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                Svc.Log.Debug(ex, "Compass: hub connection dropped.");
                failures++;
            }
            finally
            {
    ...
    private async Task ListenAsync(CancellationToken ct)
    {
        ...
        Svc.Log.Information("Compass: connected to the beacon hub.");
    ...
    private void TryQueue(string payload)
    {
        try
        {
            var evt = JsonSerializer.Deserialize<BeaconEvent>(payload, CompassJson.Options);
            if (evt is not null)
                inbox.Enqueue(evt);
        }
        catch (JsonException ex)
        {
            ...
            Svc.Log.Debug(ex, "Compass: could not read a hub event.");
        }
    }
