...
/// <summary>
/// The small drawn flourishes that make Compass look like an atlas rather than a form: rules with a
/// fleuron, a compass rose watermark, a wax seal, and the flame that carries the whole idea.
///
...
public static class Ornament
{
    ...
    /// <summary>
    /// A compass rose, drawn faintly behind a panel as a watermark. Purely decorative, so it is drawn
    /// to the background draw list and never intercepts a click.
    /// </summary>
    public static void CompassRose(ImDrawListPtr draw, Vector2 centre, float radius, Vector4 colour)
    {
        var packed = colour.Packed();
    ...
    public static void ShareCode(string shareCode, float sealDiameter)
    {
        ...
        Tooltip("Click to copy this share code. Anyone can paste it into /compass go to find this beacon.");
    ...
    /// <summary>
    /// A button in Compass's own colours. Used for the one action a panel is really about, so that
