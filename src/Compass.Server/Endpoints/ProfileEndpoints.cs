...
public static class ProfileEndpoints
{
    public static void MapProfileEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPut("/{id:guid}/availability", SetAvailabilityAsync)
            .RequireRateLimiting(RateLimits.Write)
            .WithSummary("Override the availability Compass would otherwise derive.");
    ...
    private static async Task<IResult> ReportAsync(
        Guid id,
        Compass.Shared.Beacons.ReportBeaconRequest request,
        HttpContext http,
        CompassDbContext db,
        CancellationToken ct)
    {
        ...
        if (reason.Length is 0 or > Compass.Shared.Beacons.BeaconLimits.ReportReasonMaxLength)
