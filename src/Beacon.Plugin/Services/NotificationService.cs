...
public sealed class NotificationService(Configuration config, LocationService location)
{
    ...
    public void Error(string message) => Toast("Beacon", message, NotificationType.Warning);
    ...
    private static void Echo(string message)
    {
        try
        {
            Svc.Chat.Print($"[Beacon] {message}");
        }
        catch (Exception ex)
        {
            Svc.Log.Debug(ex, "Beacon: could not echo to chat.");
        }
    }
}
