...
public sealed class TravelService(Configuration config, LocationService location) : IDisposable
{
    ...
    public JourneyPlan Plan(BeaconDto beacon)
    {
        ...
        if (!lifestream.Available)
            return Impossible("Lifestream is not installed. Beacon uses it to travel.");
        ...
        {
            return Impossible(
                $"Beacon does not know an aetheryte for {beacon.Location.ZoneName}, so it cannot teleport you there.");
        }
    ...
    public bool Begin(BeaconDto beacon, out string error)
    {
        ...
        error = string.Empty;
        Svc.Log.Information("Beacon: journey to {Beacon} begun at stage {Stage}.", beacon.Name, Stage);
        return true;
    }
    ...
    private void TickWalk()
    {
        ...
        if (distance is { } d && d <= ArrivalRadius)
        {
            ...
            Svc.Log.Information("Beacon: arrived at {Beacon}.", beacon.Name);
            return;
        }
    ...
    private void Fail(string reason)
    {
        ...
        navmesh.Stop();
        Svc.Log.Warning("Beacon: journey failed - {Reason}", reason);
    }
