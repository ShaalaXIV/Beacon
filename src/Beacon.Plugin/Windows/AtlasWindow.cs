...
public sealed class AtlasWindow : Window
{
    ...
    public override void Draw()
    {
        ...
        {
            if (ImGui.BeginTabBar("##beaconTabs"))
            {
    ...
    private void DrawDetail(BeaconDto? beacon)
    {
        ...
        // The detail pane is the atlas page: parchment, dark ink, and a cardinal rose watermark.
        Theme.PushPage();
    ...
    private void DrawLightPopup()
    {
        if (lightPopupQueued)
        {
            ImGui.OpenPopup("Light the beacon###BeaconLight");
            lightPopupQueued = false;
        }

        var open = true;
        if (!ImGui.BeginPopupModal("Light the beacon###BeaconLight", ref open, ImGuiWindowFlags.AlwaysAutoResize))
            return;
